...
public class Vehicle08Controller : MonoBehaviour
{
    ...
    public float slopeMotorSpeedBaseValue = 150f; // Base motor speed for slopes
    public float brakeForce = 2000f; // Brake force applied to the wheels
    ...
    private float currentSlopeMotorSpeed = 0f; // Motor speed depending on slope direction
    private bool isOnSlope = false; // To store whether the car is on a slope

    private bool isMovementKeyPressed = false;
    private bool isBraking = false; // To store whether the brake is applied

    public float raycastLength = 8f; // Fixed length of the raycast to detect slopes
    ...
    void Update()
    {
        ...
        // Apply brake force if space bar is pressed
        if (Input.GetKey(KeyCode.Space))
        {
            isBraking = true;
            // Set targetSpeed to 0, but use brake force to smoothly decelerate
            targetSpeed = 0f;
        }
        else
        {
        ...
        {
            targetSpeed = currentSlopeMotorSpeed; // Use slopeMotorSpeed on slope
        }
        else
        {
        ...
        if (isBraking)
        {
            // Apply brake force gradually
            float brakeFactor = Mathf.Clamp01(Mathf.Abs(currentMotorSpeed) / brakeForce);
            currentMotorSpeed = Mathf.SmoothDamp(currentMotorSpeed, targetSpeed, ref velocity, smoothTime, brakeForce * brakeFactor);
        }
        else
        {
    ...
    void CheckIfOnSlope()
    {
        ...
        if (hit.collider != null)
        {
            ...
            if (isOnSlope)
            {
                ...
                float slopeFactor = slopeAngle / maxSlopeAngle; // Slope factor based on steepness

                if (slopeDirection > 0) // Climbing up
                {
                    currentSlopeMotorSpeed = -slopeMotorSpeedBaseValue * slopeFactor; // Slow down when climbing up
                }
                else if (slopeDirection < 0) // Going down
                {
                    currentSlopeMotorSpeed = slopeMotorSpeedBaseValue * slopeFactor; // Speed up when going down
                }

                Debug.Log(currentSlopeMotorSpeed);
            }
        }
        else
        {
